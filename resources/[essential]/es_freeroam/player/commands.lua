			...
			if nom == "Citizen" and prenom == "Citizen" then
				if(args[2] ~= nil and args[3] ~= nil) then
					if (string.match(args[2], '[a-z]') and string.match(args[3], '[a-z]')) then
						...
						TriggerClientEvent("chatMessage", source,  "^3SYSTEM", {255, 255, 255},  "Nom et prénom ne doivent comporter que des lettres")
					end
				else
					TriggerClientEvent("chatMessage", source,  "^3SYSTEM", {255, 255, 255}, "Usage : /changename [Prénom] [Nom]")
				end
			else
				TriggerClientEvent("chatMessage", source,  "^3SYSTEM", {255, 255, 255}, "Vous avez déjà changé votre nom")
			end
end)
